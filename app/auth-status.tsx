...
export default async function AuthStatus() {
  ...
  return (
    <aside className={`cloud-account-chip ${user ? 'signed-in' : ''}`} aria-label="云端同步账号" tabIndex={0}>
      <Cloud aria-hidden="true" />
      <span><b>{user ? '云端账号' : '仅保存在本机'}</b><small>{user ? user.displayName : '登录后可跨设备同步'}</small></span>
